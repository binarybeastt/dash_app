...
current_index = 0
def get_next_data_chunk(chunk_size=20):
                ...
                html.Div(
                    [
                        dcc.Tabs(
                            ...
                            value='tab-1',
                            children=[
                                dcc.Tab(label='Temperature', value='tab-1', style={"font-size":"16px", "text-align": "center", **dark_mode_styles}, selected_style={"background-color": "#333", "color": "#ffffff"}),
                                dcc.Tab(label='Other Metrics (Future)', value='tab-2', style=dark_mode_styles, selected_style={"background-color": "#333", "color": "#ffffff"}),
                                dcc.Tab(label='Other Metrics (Future)', value='tab-3', style=dark_mode_styles, selected_style={"background-color": "#333", "color": "#ffffff"}),
                            ],
                            colors={
                                "border": "#333",
                                "primary": "#ff5733",
...
def update_time_series_chart(n_intervals):
...
def update_temp_humidity_chart(n_intervals):
    fig = go.Figure()
    chunk = get_next_data_chunk()
...
def update_scatter_plot(n_intervals):
    chunk = get_next_data_chunk()
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=chunk['Li'], 
...
@app.callback(
    ...
    [dash.dependencies.Input('Pm-variations', 'id')]
)
...
@app.callback(
    ...
    [dash.dependencies.Input('VOC-variations', 'id')]
)
...
@app.callback(
    ...
    [dash.dependencies.Input('No2-variations', 'id')]
)
def update_VOC_chart(_):
    fig = go.Figure()
...
@app.callback(
    ...
    [dash.dependencies.Input('temperature-led', 'id')]
)
def update_temperature_led(_):
    latest_temp = df['Tm'].iloc[-1]  
    return f"{latest_temp:.2f}"  

@app.callback(
    dash.dependencies.Output('ammonia-led', 'value'),
    [dash.dependencies.Input('ammonia-led', 'id')]
)
def update_ammonia_led(_):
    latest_nh3 = df['Nh3'].iloc[-1]  
